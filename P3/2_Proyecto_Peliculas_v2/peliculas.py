...
def modificarCaracteristicaPeliculas():
    ...
    if len(pelicula) > 0:
        for i in pelicula:
            print(f"\t{i}: {pelicula[i]}")
            resp = input(f"deseas mosdificar el valor de la caracteristica de {i}? (si/no)").lower().strip()
            if resp == "si":
                pelicula[i] = input(f"ingrese el valor que le quiere dar a {i}")
                input("\n\t\t::: ¡LA OPERACIÓN SE REALIZÓ CON ÉXITO! :::")      
    else:
     print("\t..:: No hay películas en el sistema ::..")
...
def borrarCaracteristicaPeliculas():
    ...
    if len(pelicula) > 0:
        print("Valoeres actuales: ")
        for i in pelicula:
            print(f"\n\t{i}: {pelicula[i]}")
        resp = input("Deseas borrar una caracteristica?").lower().strip()
        if resp == "si":
            caracterisitica = input("\nIngresa el nombre de la característica que deseas borrar: ").lower().strip()
            if caracterisitica in pelicula:
                pelicula.pop(caracterisitica)
                print("\n\t\t::: ¡LA OPERACIÓN SE REALIZÓ CON ÉXITO! :::")
            else:
...
#cada def hay qeu poner titulo y borrar en pantalla , en caso de ser necesario confirmas que hay peliculas 
#update sierve para poder agregar cosas a la lista lista.update({atributoAgregar:valor_Agregar})
#otra forma de agregar peli: pelicula[atributo] = valor atributo
# pelicula["nombre"] = input("ingrese el nomnbre de la pelicula ").upper().strip()
